...
# AI-персонализация (Groq API) — опционально
...
Каждое письмо генерируется через Groq API (`llama-3.3-70b-versatile` — бесплатный):
...
Если `AI_API_KEY` не задан — письма генерируются по шаблонам (как раньше).
...
internal/
  ai/                      — Groq API клиент, промпты, парсинг ответов
